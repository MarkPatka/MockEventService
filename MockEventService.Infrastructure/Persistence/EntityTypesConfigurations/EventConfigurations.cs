...
public class EventConfigurations : IEntityTypeConfiguration<Event>
{
    ...
    private static void ConfigureParticipantsTable(EntityTypeBuilder<Event> builder)
    {
        builder.OwnsMany(e => e.Participants, pb =>
        {
            ...
            pb.WithOwner().HasForeignKey("EventId");
    ...
    private static void ConfigureEventsTable(EntityTypeBuilder<Event> builder)
    {
        ...
        // Данные отзывов (обновляются черех поглощение событий ReviewService)
    ...
    private static void ConfigureLocations(EntityTypeBuilder<Event> builder)
    {
