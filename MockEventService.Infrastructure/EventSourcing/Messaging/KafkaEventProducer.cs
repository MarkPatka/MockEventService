...
public class KafkaEventProducer : IEventProducer
{
        ...
        where TMessage : class
    {
        ...
        {
            _logger.LogError(ex, "Failed to produce message to {Topic}. Sending to DLQ.", topic);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("DeadLetterTopic is not configured; skipping DLQ send.");
            return;
        }
        ...
        try
        {
            ...
            headers.Add("dlq-error", Encoding.UTF8.GetBytes(originalException.Error?.Reason ?? originalException.Message));
            ...
            await _producer.ProduceAsync(_options.DeadLetterTopic, dlqMessage, cancellationToken);
            _logger.LogInformation("Message sent to DLQ topic {DlqTopic} for key {Key}", _options.DeadLetterTopic, key);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to send message to DLQ topic {DlqTopic}. Original error: {OriginalError}",
                _options.DeadLetterTopic, originalException.Message);
        }
    }
